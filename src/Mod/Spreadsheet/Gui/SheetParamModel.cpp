...
SheetParamModel::SheetParamModel(Sheet* _sheet, QObject* parent)
    : QAbstractTableModel(parent)
    , sheet(_sheet)
{
    // NOLINTBEGIN
    cellUpdatedConnection =
        sheet->cellUpdated.connect(std::bind(&SheetParamModel::cellUpdated, this, sp::_1));
    rangeUpdatedConnection =
        sheet->rangeUpdated.connect(std::bind(&SheetParamModel::rangeUpdated, this, sp::_1));
    // NOLINTEND
...
SheetParamModel::~SheetParamModel()
{
    cellUpdatedConnection.disconnect();
    rangeUpdatedConnection.disconnect();
}

int SheetParamModel::rowCount(const QModelIndex& parent) const
{
    Q_UNUSED(parent);
    return 16384;
}

int SheetParamModel::columnCount(const QModelIndex& parent) const
{
    Q_UNUSED(parent);
    return 4; /* ParamName | ParamValue | ParamNote/Unit | ParamDesc */
}
...
QVariant SheetParamModel::data(const QModelIndex& index, int role) const
{
...
QVariant SheetParamModel::headerData(int section, Qt::Orientation orientation, int role) const
{
...
void SheetParamModel::setCellData(QModelIndex index, QString str)
{
    try {
...
bool SheetParamModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    if (role == Qt::DisplayRole) {
...
Qt::ItemFlags SheetParamModel::flags(const QModelIndex& /*index*/) const
{
    return Qt::ItemIsSelectable | Qt::ItemIsEditable | Qt::ItemIsEnabled;
}

void SheetParamModel::cellUpdated(CellAddress address)
{
...
void SheetParamModel::rangeUpdated(const Range& range)
{
...
#include "moc_SheetParamModel.cpp"
