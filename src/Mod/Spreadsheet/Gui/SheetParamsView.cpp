...
SheetParamsView::SheetParamsView(QWidget* parent)
    : QTableView(parent)
    , sheet(nullptr)
    , tabCounter(0)
{
...
SheetParamsView::~SheetParamsView() = default;

void SheetParamsView::onRecompute()
{
...
void SheetParamsView::onConfSetup()
{
    auto ranges = selectedRanges();
    if (ranges.empty()) {
        return;
...
void SheetParamsView::cellProperties()
{
...
std::vector<Range> SheetParamsView::selectedRanges() const
{
...
QModelIndexList SheetParamsView::selectedIndexesRaw() const
{
    return selectedIndexes();
}

void SheetParamsView::updateCellSpan()
{
    int rows, cols;
...
void SheetParamsView::setSheet(Sheet* _sheet)
{
    sheet = _sheet;
...
void SheetParamsView::commitData(QWidget* editor)
{
    QTableView::commitData(editor);
}

bool SheetParamsView::edit(const QModelIndex& index, EditTrigger trigger, QEvent* event)
{
    if (trigger
...
bool SheetParamsView::event(QEvent* event)
{
...
void SheetParamsView::deleteSelection()
{
...
void SheetParamsView::copySelection()
{
    _copySelection(selectedRanges(), true);
}

void SheetParamsView::_copySelection(const std::vector<App::Range>& ranges, bool copy)
{
    int minRow = INT_MAX;
    int maxRow = 0;
    int minCol = INT_MAX;
...
void SheetParamsView::cutSelection()
{
    _copySelection(selectedRanges(), false);
}

void SheetParamsView::pasteClipboard()
{
...
void SheetParamsView::finishEditWithMove(int keyPressed,
...
void SheetParamsView::ModifyBlockSelection(int targetRow, int targetCol)
{
...
void SheetParamsView::mergeCells()
{
    Gui::Application::Instance->commandManager().runCommandByName("Spreadsheet_MergeCells");
}

void SheetParamsView::splitCell()
{
    Gui::Application::Instance->commandManager().runCommandByName("Spreadsheet_SplitCell");
}

void SheetParamsView::closeEditor(QWidget* editor, QAbstractItemDelegate::EndEditHint hint)
{
    QTableView::closeEditor(editor, hint);
}

void SheetParamsView::mousePressEvent(QMouseEvent* event)
{
    tabCounter = 0;
    QTableView::mousePressEvent(event);
}

void SheetParamsView::selectionChanged(const QItemSelection& selected,
...
void SheetParamsView::edit(const QModelIndex& index)
{
    currentEditIndex = index;
    QTableView::edit(index);
}

void SheetParamsView::contextMenuEvent(QContextMenuEvent*)
{
    const QMimeData* mimeData = QApplication::clipboard()->mimeData();
...
QString SheetParamsView::toHtml() const
{
...
#include "moc_SheetParamsView.cpp"
